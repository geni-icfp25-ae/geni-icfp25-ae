...
-   Evaluate `dice` on the weather-factory benchmark:
    ```shell
    SANITI_CHECK=1 make -C weather-factory bench.dice # ~15 mins
    ```
...
-   Evaluate `gennifer` on the weather-factory benchmark:
    ```shell
    SANITI_CHECK=1 make -C weather-factory bench.gennifer # ~15 mins
    ```
    This will generate two files:
...
-   Evaluate `dice` on the grid-networks(router-failure) benchmark:
    ```shell
    SANITI_CHECK=1 make -C grid-networks router-failure.dice.csv # ~10 mins
    ```
    This will generate `grid-networks/router-failure.dice.csv`.

-   Evaluate `gennifer` on the grid-networks(router-failure) benchmark:
    ```shell
    SANITI_CHECK=1 make -C grid-networks router-failure.gennifer.csv # ~10 mins
    ```
...
-   Evaluate `gennifer` on the bayesian-networks benchmark:
    ```shell
    SANITI_CHECK=1 make -C bayesian-networks gennifer-l-i--pt-summary.csv # ~5 mins
    ```
...
`SANITI_CHECK=1` enables partial runs for faster reproduction of results. All benchmarks are included, but for each benchmark, only a subset of programs is executed—skipping a few especially time-consuming ones. For example, in the Bayesian network benchmark, `SANITI_CHECK=1` runs 5 networks, while `SANITI_CHECK=0` runs all 7. 

Set `SANITI_CHECK=0` to run the full set of programs in each benchmark, which may (1) take several days to complete and (2) require significant memory.

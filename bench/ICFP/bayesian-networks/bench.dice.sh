...
# Set SANITI_CHECK to 1 to enable, or leave unset/0 to disable
SANITI_CHECK=${SANITI_CHECK:-0}

if [[ "${SANITI_CHECK}" -eq 1 ]]; then
    warmup_runs=3
    hyperf_runs=10
else
    warmup_runs=3
...
{
    echo "Benchmarking Bayesian networks with the following settings:"
    echo "SANITI_CHECK: ${SANITI_CHECK}"

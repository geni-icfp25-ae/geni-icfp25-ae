    ...
    for x in range(1, args.n+1):
        for y in range(1, args.m+1):
            if x == 1 and y == 1:
                ...
                lines.append(f"let conn_{x}_{y} = (conn_{x}_{y-1} && flip {args.p}) in")
            elif y == 1:
                lines.append(f"let conn_{x}_{y} = (conn_{x-1}_{y} && flip {args.p}) in")
            else:
                if args.mode == "2-flow":
                    lines.append(f"let conn_{x}_{y} = ((conn_{x}_{y-1} && flip {args.p}) || (conn_{x-1}_{y} && flip {args.p})) in")
                elif args.mode == "3-flow":
                    lines.append(f"let conn_{x}_{y} = ((conn_{x}_{y-1} && flip {args.p}) || (conn_{x-1}_{y} && flip {args.p}) || (conn_{x-1}_{y-1} && flip {args.p})) in")
                else:
